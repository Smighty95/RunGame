...
public class SuperEnemy extends GameObject {
	...
	public SuperEnemy(float x, float y, ID id,Handler handler) {
		super(x, y, id);
	...
	public void tick() {
		...
		if(thereIsPlayer2==true) {
				...
				if(dist1>dist2) {
					velX=(float) ((-1.0/dist2)*difX2*3);
					velY=(float) ((-1.0/dist2)*difY2*3);
				}
				else {
					velX=(float) ((-1.0/dist1)*difX1*3);
					velY=(float) ((-1.0/dist1)*difY1*3);
				}
			}
		else {
			velX=(float) ((-1.0/dist1)*difX1*3);
